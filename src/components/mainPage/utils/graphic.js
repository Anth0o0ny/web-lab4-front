...
export function drawGraphic(width = 300, height = 300, rValue = "R", canvas, hits) {
    ...
    if (isNaN(rValue) || rValue === "R") {
        ...
        canvasRadius = rValue;
        canvasHalfRadius = Math.round((rValue / 2 + Number.EPSILON) * 100) / 100;
        if (canvasRadius > 0) {
            drawNormalGraphicsCarcass(ctx);
            drawRadius(ctx);
...
function drawNormalGraphicsCarcass(ctx) {
    ...
    /* 4 quarter - quarter circle */
    ctx.beginPath();
    ctx.moveTo(MID_WIDTH_PIX, MID_HEIGHT_PIX);
    ctx.arc(MID_WIDTH_PIX, MID_HEIGHT_PIX, RADIUS_PIX, 2*Math.PI ,Math.PI / 2, false);
    ctx.stroke();
    ctx.fill();

    /* Color of axes and points on the graph */
    ctx.globalAlpha = 1;
    ctx.fillStyle = "hsl(217, 71%, 45%)";
...
function drawMirroredGraphicsCarcass(ctx) {
    ...
    /* 1 mirrored quarter - empty */

    /* 2 mirrored quarter - quarter circle */
    ctx.beginPath();
    ctx.moveTo(MID_WIDTH_PIX, MID_HEIGHT_PIX);
    ctx.arc(MID_WIDTH_PIX, MID_HEIGHT_PIX, RADIUS_PIX, Math.PI, 3*Math.PI / 2, false);
    ctx.stroke();
    ctx.fill();

    /* 3 mirrored quarter - rectangle  */
    ctx.beginPath();
    ctx.moveTo(MID_WIDTH_PIX, MID_HEIGHT_PIX);
    ctx.lineTo(MID_WIDTH_PIX, MID_HEIGHT_PIX + RADIUS_PIX);
    ctx.lineTo(MID_WIDTH_PIX - RADIUS_PIX, MID_HEIGHT_PIX + RADIUS_PIX);
